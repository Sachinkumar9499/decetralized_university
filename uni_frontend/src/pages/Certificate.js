...
const Certificate = () => {
  ...
  const handleIssueCertificate = async () => {
    if (!studentName || !courseName) {
      alert("⚠️ Please enter both student name and course name.");
      return;
    }
  ...
  const handleGetCertificate = async () => {
    if (!queryName) {
      alert("⚠️ Please enter a student name to query.");
      return;
    }

    try {
      const result = await backend_backend.get_certificate(queryName);
